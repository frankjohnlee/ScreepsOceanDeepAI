...
module.exports = {
    ...
    run: function(curr_room, x_rec = null, y_rec = null, current_try = null){
        ...
        if ((curr_room.createConstructionSite(x, y, STRUCTURE_EXTENSION) == ERR_RCL_NOT_ENOUGH) || (curr_room.createConstructionSite(x,y) == ERR_FULL)) {
            var do_nothing = 0;
        }
        // IF SOMETHING IS WRONG WITH THE FORMATTING
        else if (curr_room.createConstructionSite(x, y, STRUCTURE_EXTENSION) == ERR_INVALID_ARGS) {
        ...
        // if there are no error then I think the construction site was successfully set up
        else if (curr_room.createConstructionSite(x, y, STRUCTURE_EXTENSION) == ERR_INVALID_TARGET) {
            ...
            while (curr_room.createConstructionSite(loop_x, loop_y, STRUCTURE_EXTENSION) == ERR_INVALID_TARGET
                ){
                if (loop_x - x < 14){
                    loop_x ++;
                    loop_x_2++;
                    if (curr_room.createConstructionSite(loop_x, loop_y, STRUCTURE_EXTENSION) == ERR_INVALID_TARGET){
                        loop_y++;
                        loop_y_2++;
                    }
                }
