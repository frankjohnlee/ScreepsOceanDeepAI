module.exports = {
    run: function (curr_spawn, creep_list, current_spawn_level) {
        ...
        const harvesterSpawn = creep_list.length == 0 || creep_list == undefined;
        if (harvesterSpawn){
            var body_parts_list = require('createBalancedCreep').run(energy);
            console.log(spaces + spaces + spaces + spaces + spaces + "Emergency: Need more harvesters. Will create with whatever energy I have:  " + avail_energy);
            ...
            curr_spawn.createCreep([WORK, WORK, WORK, CARRY, CARRY, CARRY, MOVE, MOVE, MOVE], undefined, {
                role: 'Harvester',
                working: false,
                home_room: curr_spawn.room.name,
                home_spawn: curr_spawn
            });
            curr_spawn.createCreep([WORK, WORK, CARRY, CARRY, MOVE, MOVE], undefined, {
                role: 'Harvester',
                working: false,
                home_room: curr_spawn.room.name,
                home_spawn: curr_spawn
            });
            curr_spawn.createCreep([WORK, WORK, CARRY, CARRY, MOVE], undefined, {
                role: 'Harvester',
                working: false,
                home_room: curr_spawn.room.name,
                home_spawn: curr_spawn
            });
            curr_spawn.createCreep([WORK, WORK, CARRY, MOVE], undefined, {
                role: 'Harvester',
                working: false,
                home_room: curr_spawn.room.name,
                home_spawn: curr_spawn
        ...
        if (spawnTurnedOn) {
            ...
            if (current_number_of_transferer < min_number_transferer) {
